...
def check_horse_winner(game):
    keep_playing = []
    for i, player in enumerate(game):
        if player != 'HORSE':
            keep_playing.append(str(i))

    if len(keep_playing) == len(game):
        return 'Everyone: keep playing!'
    elif len(keep_playing) > 1:
        return 'Players {0}: keep playing!'.format(', '.join(keep_playing))
    else:
        return 'Player {0} wins!'.format(keep_playing[0])
